...
def _read_oat_features(feature_map, num_steps: int) -> tuple[np.ndarray, np.ndarray]:
    if "steps/oat_latents" not in feature_map or "steps/oat_latent_mask" not in feature_map:
        return (
            np.zeros((num_steps, 0, 0), dtype=np.float32),
            np.zeros((num_steps, 0), dtype=bool),
        )

    flat_mask = np.asarray(feature_map["steps/oat_latent_mask"].int64_list.value, dtype=np.int64)
    if num_steps == 0:
        return (
            np.zeros((0, 0, 0), dtype=np.float32),
            np.zeros((0, 0), dtype=bool),
        )
    if flat_mask.size % num_steps != 0:
        raise ValueError("OAT latent mask size is not divisible by the number of steps.")
    num_queries = flat_mask.size // num_steps
    latent_mask = flat_mask.reshape(num_steps, num_queries).astype(bool)

    flat_latents = np.asarray(feature_map["steps/oat_latents"].float_list.value, dtype=np.float32)
    if num_queries == 0:
        return np.zeros((num_steps, 0, 0), dtype=np.float32), latent_mask
    if flat_latents.size % (num_steps * num_queries) != 0:
        raise ValueError("OAT latent tensor size is not divisible by num_steps * num_queries.")
    latent_dim = flat_latents.size // (num_steps * num_queries)
    oat_latents = flat_latents.reshape(num_steps, num_queries, latent_dim)
    return oat_latents, latent_mask
...
class _Episode:
    ...
    oat_latents: np.ndarray
    oat_latent_mask: np.ndarray
...
def _parse_episode(raw_example: bytes) -> _Episode:
    ...
    oat_latents, oat_latent_mask = _read_oat_features(feature_map, num_steps)
                ...
                for raw_record in records:
                    ...
                    for step_idx in indices:
                        ...
                        yield {
                            ...
                            "oat_latents": np.asarray(episode.oat_latents[step_idx], dtype=np.float32),
                            "oat_latent_mask": np.asarray(episode.oat_latent_mask[step_idx], dtype=bool),
                        }
            if not repeat:
                return
...
def _collate_batch(samples: list[dict]) -> dict:
    ...
    batch = {
        ...
        "oat_latents": np.stack([sample["oat_latents"] for sample in samples], axis=0),
        "oat_latent_mask": np.stack([sample["oat_latent_mask"] for sample in samples], axis=0),
...
class LiberoOatRldsDataset:
    def __init__(
        ...
        *,
        ...
        logging.info(f"Preparing {len(datasets)} LIBERO OAT RLDS datasets...")
