...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Augment LIBERO RLDS datasets with OAT latent targets.")
    parser.add_argument(
        "--source-rlds-root",
        type=Path,
        required=True,
        help="Root directory containing the source RLDS datasets.",
    )
    parser.add_argument(
        "--source-libero-root",
        type=Path,
        required=True,
        help="Root directory containing raw LIBERO HDF5 files. Used to rewrite episode metadata file paths.",
    )
    parser.add_argument(
        "--output-rlds-root",
        type=Path,
        required=True,
        help="Root directory where the OAT-augmented RLDS datasets will be written.",
    )
    parser.add_argument(
        "--oat-root",
        type=Path,
...
def _build_file_index(libero_root: Path) -> dict[str, str]:
