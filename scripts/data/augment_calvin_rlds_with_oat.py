...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Augment a CALVIN RLDS dataset with OAT latent targets.")
    parser.add_argument(
        "--source-rlds-root",
        type=Path,
        required=True,
        help="Root directory containing the source CALVIN RLDS dataset.",
    )
    parser.add_argument(
        "--output-rlds-root",
        type=Path,
        required=True,
        help="Root directory where the OAT-augmented CALVIN RLDS dataset will be written.",
    )
    parser.add_argument(
        "--dataset-name",
        default="calvin",
    ...
    parser.add_argument(
        "--action-feature",
        default="steps/action",
        choices=("steps/action", "steps/original_actions"),
        help="Serialized TF Example feature to use for OAT latent extraction.",
    )
    parser.add_argument(
        "--action-horizon",
        type=int,
...
def _load_oat_tokenizer(oat_root: Path, checkpoint: Path, device: str):
    try:
        ...
        raise ModuleNotFoundError(
            "Missing OAT tokenizer dependencies. Initialize external/oat and its environment."
        ) from exc
...
def _resolve_version_dir(root: Path, dataset_name: str, version: str) -> Path:
    candidate = root / dataset_name / version
    if candidate.exists():
        return candidate
    candidate = root / version
    if candidate.exists():
        return candidate
    raise FileNotFoundError(f"Could not find dataset version dir under {root} for {dataset_name}:{version}.")
...
def _extract_actions(example: tf.train.Example, action_feature: str) -> np.ndarray:
    features = example.features.feature
    traj_len = len(features["steps/is_first"].int64_list.value)
    flat_actions = np.asarray(features[action_feature].float_list.value, dtype=np.float32)
    if traj_len == 0:
        raise ValueError("Encountered empty trajectory while extracting actions.")
    if flat_actions.size % traj_len != 0:
        raise ValueError(
            f"Action feature '{action_feature}' length {flat_actions.size} is not divisible by trajectory length {traj_len}."
        )
...
def _augment_serialized_example(
    ...
    *,
    ...
    actions = _extract_actions(example, action_feature)
...
def _get_source_shard_paths(source_version_dir: Path, source_builder, split: str) -> list[Path]:
    shard_paths = sorted(source_version_dir.glob(f"{source_builder.info.name}-{split}.tfrecord-*"))
    if not shard_paths:
        raise FileNotFoundError(f"No TFRecord shards found for split '{split}' under {source_version_dir}")
    return shard_paths
...
def _write_augmented_dataset(
    ...
    *,
    ...
    shard_paths = _get_source_shard_paths(source_version_dir, source_builder, split)
    serialized_examples = _iter_serialized_examples(shard_paths, max_episodes=max_episodes)

    split_info = source_builder.info.splits[split]
    ...
    shard_template = f"{source_builder.info.name}-{split}.tfrecord-{{:05d}}-of-{num_shards:05d}"
    ...
    try:
        ...
        for serialized_example in tqdm(
            serialized_examples, desc=f"Converting {source_builder.info.name}:{split}", unit="episode"
        ):
...
def main() -> None:
    ...
    source_version_dir = _resolve_version_dir(args.source_rlds_root, args.dataset_name, args.version)
    output_version_dir = args.output_rlds_root / args.dataset_name / args.version
    ...
    for split in args.splits:
        current_num_queries, current_latent_dim = _write_augmented_dataset(
            source_builder,
            source_version_dir,
            output_version_dir,
            tokenizer,
        ...
        if num_queries is None or latent_dim is None:
            ...
            raise RuntimeError(
                "Encountered inconsistent OAT latent shape across splits: "
                f"expected {(num_queries, latent_dim)}, got {(current_num_queries, current_latent_dim)}."
            )
    ...
    print(
        f"[done] {args.dataset_name}: wrote OAT-augmented RLDS to {output_version_dir} "
        f"(splits={args.splits}, action_feature={args.action_feature}, "
